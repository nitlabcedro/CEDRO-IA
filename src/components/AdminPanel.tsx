...
export default function AdminPanel({ 
  ...
  return (
    <div className="space-y-8 w-full max-w-none px-0 py-4 font-sans">
                          ...
                          <div className="flex items-center flex-wrap sm:justify-end gap-2 w-full sm:w-auto">
                            ...
                            {isCurrentUserAdmin && (
                              <button
                                ...
                                className="inline-flex items-center justify-center rounded-xl border border-[#075618]/20 bg-[#075618]/5 px-3 py-2 text-xs font-bold text-[#075618] transition-all hover:bg-[#075618]/10 cursor-pointer"
                              >
                                Redefinir status
                              </button>
                            )}
              ...
              <div className="p-6 sm:p-8 space-y-6">
                ...
                <div className="flex items-start gap-4 border-b border-slate-100 pb-5">
                  <div className="size-12 rounded-xl bg-amber-50 text-amber-700 border border-amber-200 flex items-center justify-center shrink-0 shadow-3xs">
                    <RefreshCw size={24} className="text-amber-600 animate-spin-slow" />
                  </div>
                  <div>
                ...
                {/* Resumo da IA */}
                <div className="grid grid-cols-2 gap-4 p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-700">
                  <div>
                    <span className="font-bold text-slate-500 uppercase block text-[10px]">Nome da IA</span>
                    <span className="font-semibold text-slate-900 text-sm block truncate" title={resetStatusRecord.nomeFerramenta}>{resetStatusRecord.nomeFerramenta}</span>
                  </div>
                  <div>
                    <span className="font-bold text-slate-500 uppercase block text-[10px]">ID/Protocolo</span>
                    <span className="font-mono text-slate-900 block truncate" title={resetStatusRecord.id}>{resetStatusRecord.id}</span>
                  </div>
                  <div>
                    <span className="font-bold text-slate-500 uppercase block text-[10px]">Setor solicitante</span>
                    <span className="font-semibold text-slate-900 block truncate" title={resetStatusRecord.unidadeSetor}>{resetStatusRecord.unidadeSetor}</span>
                  </div>
                  <div>
                    <span className="font-bold text-slate-500 uppercase block text-[10px]">Status atual</span>
                    <span className="font-semibold text-slate-900 block truncate" title={resetStatusRecord.statusUso || "Não informado"}>{resetStatusRecord.statusUso || "Não informado"}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-700 uppercase tracking-wide flex items-center justify-between">
                    <span>Justificativa da redefinição <span className="text-rose-500">*</span></span>
                  </label>
                  <textarea 
                    value={resetReason}
                    ...
                    className="w-full h-24 bg-white border border-slate-250 text-slate-800 placeholder-slate-400 rounded-xl p-3.5 text-xs font-medium focus:border-[#075618] focus:ring-1 focus:ring-[#075618]/10 outline-none transition-all resize-none shadow-3xs"
                  />
                  {!resetReason.trim() && (
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <button 
                    ...
                    className="flex-1 py-2.5 bg-[#075618] hover:bg-[#03440c] disabled:opacity-50 disabled:bg-slate-300 disabled:border-slate-300 text-white font-bold uppercase text-[10px] tracking-wide rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-xs"
                  >
                    {isResetting ? (
                      <>
